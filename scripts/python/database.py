...
def init_db(memory_dir):
    ...
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_messages_session_ts ON messages(session_id, timestamp ASC, id ASC)')
...
def get_all_sessions(db_path):
    ...
    conn = get_connection(db_path)
    cursor = conn.execute('SELECT id, title, updated_at as mtime FROM sessions ORDER BY updated_at DESC')
    return [{"id": row["id"], "title": row["title"], "mtime": row["mtime"]} for row in cursor.fetchall()]
...
def set_session_token_limit(db_path, session_id, limit):
    conn = get_connection(db_path)
    conn.execute('UPDATE sessions SET token_limit = ? WHERE id = ?', (limit, session_id))
    conn.commit()
...
def get_messages(db_path, session_id):
    ...
    return [{"id": row["id"], "role": row["role"], "content": row["content"]} for row in cursor.fetchall()]
